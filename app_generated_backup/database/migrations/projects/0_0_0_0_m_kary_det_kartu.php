...
class mkarydetkartu extends Migration
{
    ...
    public function up()
    {
        Schema::create($this->tableName, function (Blueprint $table) {
            ...
            $table->string('ktp_no',25);
            $table->string('ktp_foto');
            $table->string('pas_foto');
            $table->string('kk_no',25);
            $table->string('kk_foto');
            $table->string('npwp_no',25);
            $table->string('npwp_foto');
            $table->date('npwp_tgl_berlaku');
            $table->bigInteger('bpjs_tipe_id')->comment('{"src":"m_general.id"}');
            $table->string('bpjs_no',30);
            $table->string('bpjs_foto');
