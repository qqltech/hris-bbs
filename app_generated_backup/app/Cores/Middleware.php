    ...
    public function handle( Request $request, Closure $next ) 
    {
        ...
        if($modelInstance){
            $modelInstance->addGlobalScope('global', function (Builder $builder) use($modelName, $modelInstance, $user) {
                if(in_array('m_comp_id', $modelInstance->columns) && @$modelInstance->getTable() != 'm_general'){
                    $builder->where("$modelName.m_comp_id", $user->m_comp_id);
                }
            });
            if (!isset($requestData['m_dir_id'])) {
                $requestData['m_dir_id'] = $user->m_dir_id;
            }
            $requestData['m_comp_id'] = $user->m_comp_id;
        }
        if(app()->request->header('Source') === 'mobile') {
            $modelInstance->addGlobalScope('global', function (Builder $builder) use($modelName, $modelInstance, $user) {
                if(in_array('creator_id', $modelInstance->columns) && !in_array(@$modelInstance->getTable(),['generate_approval','generate_approval_det','m_kary','default_users','m_general','m_dir','m_zona','m_lokasi','m_divisi','m_dept','m_posisi','m_spd','presensi_app_version'])){
