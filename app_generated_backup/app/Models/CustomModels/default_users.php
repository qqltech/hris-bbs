...
class default_users extends \App\Models\BasicModels\default_users
{
    ...
    public function scopePic($model){
        ...
        return $model->whereRaw("default_users.id = ? or default_users.id in (select u.id from default_users u join m_kary k on k.id = u.m_kary_id where k.atasan_id = default_users.id)", [auth()->user()->id ?? 0]);
    }
}
