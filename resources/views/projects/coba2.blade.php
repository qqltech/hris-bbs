@verbatim
<!-- asdadsdsaaaaaaa123editocba ajacobacoba -->
<div class="space-y-5 p-6">
  <!-- Header -->
  <div class="lg:h-[300px] 2xl:h-[500px] lg:flex flex-col lg:flex-row gap-4  rounded-xl md:space-y-10 lg:space-y-0 ">
  <!-- Bagian Kiri -->
    ...
    <div class="h-full bg-white border-t-4 border-blue-500 py-4 rounded-lg px-3">
      <div class="flex justify-between p-2">
        <FieldX 
          ...
          class="!mt-0" 
          :value="data.month"
          :errorText="formErrors.month ? 'failed' : ''" 
          @input="v => {
            data.month = v
            changeCalendar(v)
          }" 
          :hints="formErrors.month" 
      ...
      <div class="h-[85%]"> 
        ...
        <tr v-for="row in calendarRows" :key="row" class="rounded-2xl rounded-2xl">
          <td 
            v-for="cell in row" 
            :key="cell.date"
            :class="{ 'bg-blue-500 text-white': cell.isToday, 'cursor-pointer': cell.day }" 
  ...
  <div class="flex-1 h-full">
    <!-- <TableApi ref="apiTable" :api="landing.api" :columns="landing.columns">
      <template #header>
...
<div class=" bg-white rounded-2xl p-6">
      ...
      <div class="grid grid-cols-2 gap-4">
        ...
        <div class="w-full border-2 rounded-2xl p-4 mt-10">
          ...
          <div  class="mt-4">
            <!-- <button @click="addDetail" type="button"  class="bg-blue-600 hover:bg-blue-500 text-white p-2 px-4 flex items-center justify-center rounded">
              + Tambah Proyek
         </button> -->
            ...
            <table class="w-full overflow-x-auto">
              ...
              <tbody>
                <tr v-for="(item, i) in detail.items" :key="i" 
                  :class="{'bg-gray-200': proyekPilih === item.m_proyek_id}" class="border-t cursor-pointer "  @click="clickProyek(item.m_proyek_id , i)">
                  <td class="p-2 text-center pt-7 pb-4 border border-[#CACACA] ">
                    {{ i + 1 }}.
                  </td>
                  <td class="p-2 pt-7 pb-4 border border-[#CACACA]" >
                    <FieldSelect :bind="{disabled:true ,clearable: true }" class="col-span-8 !mt-0 w-full"
                ...
                <tr v-for="(item, i) in detail2.items" :key="i" class="border-t">
                  ...
                  <td class="p-2 pt-7 pb-4 border border-[#CACACA]">
                    <FieldX :bind="{ readonly: false }" @input="(v) => item.task = v" :value="item.task"
                      :error-text="formErrors.task"  placeholder="Tuliskan Task" label=''
                      class="col-span-8 !mt-0 w-full" :check="false" />
                  </td>
                  <td class="p-2 pt-7 pb-4 border border-[#CACACA]">
                    <FieldSelect :bind="{clearable:false }" :value="item.status"
