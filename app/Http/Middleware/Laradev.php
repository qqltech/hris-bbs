...
class Laradev
{
    ...
    {
        if ( !($devToken=$request->header('developer-token')) && $request->header('laradev')==null || $request->header('laradev')!=env("LARADEVPASSWORD","bismillah") ) {
        ...
        if( !in_array($devToken, $frontenders) && !in_array($devToken, $backenders) && !in_array($devToken, $owners) ){
            return response()->json(['status'=>'unauthorized'], 401);
        }
        ...
        }elseif( in_array($devToken, $owners) ){
