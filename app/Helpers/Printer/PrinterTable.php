...
class PrinterTable
{
    public function getOutputPolos($metaHeaders, $data, $border = null){
        return $this->getOutput($metaHeaders, $data,  [
            'intersection' => '',
            'horizontal' => '',
            'vertical' => config('isPrinting')?' ':' ',
        ], true);
    }
    public function getOutput($metaHeaders, $data=[], $border = null, $hideHeader=false){
        $replacement = " ";
        $tbl = $this;
        $tbl->_border = $border??[
            'intersection' => '+',
            'horizontal' => config('isPrinting')?chr(196):'-',
            'vertical' =>config('isPrinting')?chr(179): '|',
        ];
        $newData = [];
        foreach( $data AS $rowIdx => $row ){
            ...
            foreach( $temp as $tempIdx => $tempVal ){
                for( $i=count($tempVal); $i<$maxRows; $i++ ){
                    $temp[$tempIdx][]='';
                }
            }
            ...
            foreach( $temp as $tempIdx => $tempVal ){
                foreach( $tempVal as $colFixIdx => $colFix ){
                    ...
                    $newData[$newDataIdx][$colFixIdx][$tempIdx] = $colFix;//str_pad($colFix, $colLength, $replacement, $padAlign);
                }
            }
        }

        $headers = array_map( function($dt)use($replacement){
            $padAlign = STR_PAD_RIGHT;
            if( @$dt['header_align']=='center' ){
                $padAlign = STR_PAD_BOTH;
            }elseif( @$dt['header_align']=='right' ){
                $padAlign = STR_PAD_LEFT;
            }
        ...
        if( count($data) == 0){
            return implode('   ', $this->_headers[0]);
        }
    ...
    function _getSeparator()
    {
        ...
        if(config('isPrinting')){
            $row_begin = str_replace("+", chr(195) /*┌*/, $row_begin);
            $row_end = str_replace("+", chr(180) /*┐*/, $row_end);
            $implode_char = str_replace("+",'+', $implode_char);
        }
