  ...
  // ------------------------------ PERSIAPAN
  onBeforeMount(()=>{
    document.title = 'Laporan Recruitment'
  })
  ...
  const onGenerate = async () => {
    ...
    if(values.tipe?.toLowerCase() !== 'html'){
      ...
      window.open(`${store.server.url_backend}/web/report_rekrutmen` + '?' + paramsGet)
    }else{
      await fetch(`${store.server.url_backend}/web/report_rekrutmen` + '?' + paramsGet, {
        headers: {
            'Content-Type': 'html',
          },
      })
