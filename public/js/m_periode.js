...
// ------------------------------ PERSIAPAN
const endpointApi = '/m_menu'
onBeforeMount(()=>{
  document.title = 'Master Pengaturan Periode'
})
...
const onGenerate = async () => {
  ...
  if(values.tgl){
    ...
    if(values.tgl2){      
      ...
      tempWhere.push(`m_menu.created_at BETWEEN '${tempDate}' AND '${tempDate2}'`)
    }else{ 
      tempWhere.push(`m_menu.created_at >= '${tempDate}'`)
    }
  }
  if(values.tgl2){
  ...
  const response = await fetch(`${store.server.url_backend}/operation/m_menu` + '?' + new URLSearchParams({where: paramsWhere}), {
        headers: {
          'Content-Type': 'Application/json',
          Authorization: `${store.user.token_type} ${store.user.token}`
        },
