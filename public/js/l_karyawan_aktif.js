  ...
  // ------------------------------ PERSIAPAN
  onBeforeMount(()=>{
    document.title = 'Laporan Karyawan Aktif'
  })
  ...
  const onGenerate = async () => {
    ...
    if(values.tipe?.toLowerCase() !== 'html'){
      ...
      window.open(`${store.server.url_backend}/web/report_karyawan` + '?' + paramsGet)
    }else{
      await fetch(`${store.server.url_backend}/web/report_karyawan` + '?' + paramsGet, {
        headers: {
            'Content-Type': 'html',
          },
      })
