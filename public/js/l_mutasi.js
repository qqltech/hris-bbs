  ...
  const onGenerate = async () => {
    ...
    if(values.tipe?.toLowerCase() !== 'html'){
      ...
      window.open(`${store.server.url_backend}/web/report_mutasi` + '?' + paramsGet)
    }else{
      await fetch(`${store.server.url_backend}/web/report_mutasi` + '?' + paramsGet, {
        headers: {
            'Content-Type': 'html',
          },
      })
