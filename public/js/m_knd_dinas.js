...
// ------------------------------ PERSIAPAN
const endpointApi = '/m_knd_dinas'
onBeforeMount(()=>{
  document.title = 'Master Kendaraan Dinas'
})
...
async function onSave() {
  //values.tags = JSON.stringify(values.tags)
      try {
        ...
        values.is_cuti_bersama = values.is_cuti_bersama ? 1 : 0
        values.is_potong_cuti = values.is_potong_cuti ? 1 : 0
        ...
        if (!res.ok) {
          if ([400, 422].includes(res.status)) {
            ...
            throw (responseJson.errors.length ? responseJson.errors[0] : responseJson.message || "Maaf data belum valid, silahkan dikoreksi")
          } else {
            throw ("Maaf data belum valid, silahkan dikoreksi")
          }
        }
...
const landing = reactive({
  ...
  api: {
    ...
    params: {
      simplest: true,
      searchfield:'this.id, m_lokasi.nama, this.nama, this.desc',
