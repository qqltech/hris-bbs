  ...
  // ------------------------------ PERSIAPAN
  const endpointApi = '/m_jam_kerja'
  onBeforeMount(()=>{
    document.title = 'Master Jam Kerja'
  })
...
const values = reactive({
  ...
  values.direktorat = store.user.data?.direktorat
    ...
    checkedState.value = values.hari_berikutnya
  })
...
async function onSave() {
  ...
  const landing = reactive({
    ...
    api: {
      ...
      params: {
        simplest: true,
        searchfield:'this.id, this.kode, this.waktu_mulai, this.waktu_akhir, this.desc, tipe_jam_kerja.value, is_active',
      },
      onsuccess(response) {
    ...
    },
    {
      ...
      cellClass: [ 'border-r', '!border-gray-200', 'justify-start'],
      cellRenderer: ( params ) => {
        return params.data.waktu_mulai + '-' + params.data.waktu_akhir
      }
    },
    {
      headerName: 'Hari Lain',
      field: 'is_hari_berikutnya',
      filter: true,
      sortable: true,
      filter: 'ColFilter',
      resizable: true,
      ...
      cellRenderer: ({ value }) => {
        return value === true
          ? `<span class="text-green-500 rounded-md text-xs font-medium px-4 py-1 inline-block capitalize">Ada</span>`
          : `<span class="text-red-500 rounded-md text-xs font-medium px-4 py-1 inline-block capitalize">Tidak Ada</span>`
      }
    },
        {
