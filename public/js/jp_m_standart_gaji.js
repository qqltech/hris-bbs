...
// ------------------------------ PERSIAPAN
const endpointApi = '/m_standart_gaji'
onBeforeMount(()=>{
  document.title = 'Master Standart Gaji'
})
...
onBeforeMount(async () => {
  ...
  if (isRead) {
    ...
    try {
      ...
      initialValues.m_standart_gaji_det?.forEach((items)=>{
        items.__id = ++_id
...
async function onSave() {
  //values.tags = JSON.stringify(values.tags)
      try {
        ...
        values.m_standart_gaji_det = detailArr.value
...
const landing = reactive({
  ...
  api: {
    ...
    params: {
      simplest: true,
      searchfield:'this.kode, m_zona.nama, m_posisi.desc_kerja',
    },
    onsuccess(response) {
