...
const removeDetail2 = async (index) => {
  try {
    ...
    if (!itemToDelete.id) {
      // Jika tidak ada ID, hapus langsung dari array tanpa memanggil API
...
async function onSave() {
  ...
  try {
    ...
    await swal.fire({
      icon: 'success',
      text: 'Data Telah Berhasil Dimuat Ulang!'
    });
...
const handleDateClick = async (date) => {
  if (date) {
  selectedDate.value = date; // Atur tanggal yang dipilih
...
onMounted(() => {
  ...
  watchEffect(async () => {
    if (data.date) {
      // Reset proyek setiap kali tanggal berubah
      proyekPilih.value = null;
      activeProyekIndex.value = null;
      proyekNama.value = '';
      detail.items = [];
