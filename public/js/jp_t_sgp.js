...
// ------------------------------ PERSIAPAN
const endpointApi = '/t_sgp'
onBeforeMount(()=>{
  document.title = 'Surat Penghargaan atau Surat Peringatan (SP)'
})
...
async function onSave() {
  // Check if file_dokumen is empty
  if (!values.file_dokumen) {
    isBadForm.value = true;
    swal.fire({
      icon: 'error',
      text: 'File dokumen masih kosong.'
    });
    return;
  }
...
const landing = reactive({
  ...
  api: {
    ...
    params: {
      simplest: true,
      join: true,
      searchfield: 'm_kary.nik, m_kary.nama_panggilan, m_dir.nama, tgl, tipe_sgp.value, keterangan, status', // Masukkan semua field yang ada di kolom
    },
    onsuccess(response) {
