  ...
  // ------------------------------ PERSIAPAN
  onBeforeMount(()=>{
    document.title = 'Laporan Ganjaran / Surat Penghargaan / Surat Peringatan (SP)'
  })
  ...
  const onGenerate = async () => {
    ...
    if(values.tipe?.toLowerCase() !== 'html'){
      ...
      window.open(`${store.server.url_backend}/web/report_sp` + '?' + paramsGet)
    }else{
      await fetch(`${store.server.url_backend}/web/report_sp` + '?' + paramsGet, {
        headers: {
            'Content-Type': 'html',
          },
      })
