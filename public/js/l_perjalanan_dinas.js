  ...
  // ------------------------------ PERSIAPAN
  onBeforeMount(()=>{
    document.title = 'Laporan Perjalanan Dinas'
  })
  ...
  const onGenerate = async () => {
    ...
    if(values.tipe?.toLowerCase() !== 'html'){
      ...
      window.open(`${store.server.url_backend}/web/report_spd` + '?' + paramsGet)
    }else{
      await fetch(`${store.server.url_backend}/web/report_spd` + '?' + paramsGet, {
        headers: {
            'Content-Type': 'html',
          },
      })
